...
export function getSortedPostsData() {
  ...
  useEffect(() => {
    ...
    setTextUrl(process.env.NEXT_PUBLIC_VERCEL_URL ?? "url not found");
    setTextUrl2(process.env.VERCEL_URL ?? "url not found");
...
export function getCategories() {
  ...
  const allPostsData = fileNames.forEach((fileName) => {
    ...
    // Add category to list, if available
    if (matterResult?.data?.categories) {
      allCategories.push(...matterResult.data.categories);
    }
  });
